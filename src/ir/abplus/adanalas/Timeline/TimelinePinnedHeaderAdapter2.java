...
public class TimelinePinnedHeaderAdapter2 extends SectionedBaseAdapter{
    ...
    public TimelinePinnedHeaderAdapter2(Context context, Resources resources, int layoutWidth)
    {
...
//            holder.amount.setOnClickListener(this);
        //adding padding to amount shape minSize
        if (items[section][position].isExpence) {
    ...
    class SelectedViewHolder{
        ImageView icon;
        LinearLayout tagsVerticalLayout;
        AutoResizeTextView amount;
    }
                ...
                if (holder.tagsLayout != null) {
                    ...
                    if (items[section][position].tags.get(0) != null) {
                        if (items[section][position].tags.size() > 0) {
                            ...
                            Log.d("layout size is:",layoutWidth+"");
