...
public class PersianTimePickerDialog extends DialogFragment implements RadialPickerLayout.OnValueSelectedListener {
	...
	private String mPersianAMText = "ق.ظ";
	private String mPersianPMText = "ب.ظ";
	...
	public static PersianTimePickerDialog newInstance(OnTimeSetListener callback,
			int hourOfDay, int minute, boolean is24HourMode, int mMode) {
		return newInstance(callback, hourOfDay, minute, is24HourMode, true, mMode);
	}

	public static PersianTimePickerDialog newInstance(OnTimeSetListener callback,
			int hourOfDay, int minute, boolean is24HourMode, boolean vibrate, int mMode) {
		PersianTimePickerDialog ret = new PersianTimePickerDialog();
		try {
			ret.initialize(callback, hourOfDay, minute, is24HourMode, vibrate, mMode);
	...
	public void initialize(OnTimeSetListener callback,
			int hourOfDay, int minute, boolean is24HourMode, boolean vibrate, int mMode) throws IllegalAccessException {
		mCallback = callback;

		System.out.println("initialize: "+hourOfDay+" "+minute);
		
		if(mMode != PERSIAN && mMode != ENGLISH)
			throw new IllegalAccessException("Invalid mode.");
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		String[] amPmTexts = null;
		if(mMode == PERSIAN)
		{
			amPmTexts = new String[2];
			amPmTexts[0] = mPersianAMText;
			amPmTexts[1] = mPersianPMText;
		}
		else if(mMode == ENGLISH)
		{
		...
		mTimePicker.setYpeface(mTypeface);
		mTimePicker.setMode(mMode);
		mTimePicker.setPersianAMText(mPersianAMText);
		mTimePicker.setPersianPMText(mPersianPMText);
		mTimePicker.setOnValueSelectedListener(this);
		mTimePicker.setOnKeyListener(keyboardListener);
		mTimePicker.initialize(getActivity(), mInitialHourOfDay, mInitialMinute, mIs24HourMode, mVibrate, mPersianAMText, mPersianPMText, mTypeface);
		...
		mMinuteSpaceView.setTypeface(mTypeface);
	 ...
	 */
	@Override
	public void onValueSelected(int pickerIndex, int newValue, boolean autoAdvance, boolean released) {
		if (pickerIndex == HOUR_INDEX) {
			...
			setMinute(newValue);
			if(mCloseOnSingleTapMinute && released) {
				onDoneButtonClick();
			}
	...
	private void setHour(int value, boolean announce) {
		...
		if(mMode == PERSIAN)
			text = Utils.toPersianNumbers(String.format(format, value));
		else if(mMode == ENGLISH)
	...
	private void setMinute(int value) {
		...
		if(mMode == PERSIAN)
		{
			text = Utils.toPersianNumbers(String.format(Locale.getDefault(), "%02d", value));
		}
		else if(mMode == ENGLISH)
		{
	...
	private void setCurrentItemShowing(int index, boolean animateCircle, boolean delayLabelAnimate,
		...
		if (index == HOUR_INDEX) {
			...
			if(mMode == PERSIAN)
				mTimePicker.setContentDescription(Utils.toPersianNumbers(mHourPickerDescription + ": " + hours));
			else if(mMode == ENGLISH)
			...
			if(mMode == PERSIAN)
				mTimePicker.setContentDescription(Utils.toPersianNumbers(mMinutePickerDescription + ": " + minutes));
			else if(mMode == ENGLISH)
			...
			if (mInKbMode) {
				if (!mTypedTimes.isEmpty()) {
					...
					if (deleted == getAmOrPmKeyCode(AM)) {
						...
						if(mMode == PERSIAN)
							deletedKeyStr = Utils.toPersianNumbers(String.format("%d", getValFromKeyCode(deleted)));
						else if(mMode == ENGLISH)
							deletedKeyStr = String.format("%d", getValFromKeyCode(deleted));
					}
					if(mMode == PERSIAN)
						Utils.tryAccessibilityAnnounce(mTimePicker,
								Utils.toPersianNumbers(String.format(mDeletedKeyFormat, deletedKeyStr)));
					else if(mMode == ENGLISH)
						Utils.tryAccessibilityAnnounce(mTimePicker,
	...
	private void updateDisplay(boolean allowEmptyDisplay) {
		if (!allowEmptyDisplay && mTypedTimes.isEmpty()) {
			...
			if(mMode == PERSIAN)
			{
				mHourView.setText(Utils.toPersianNumbers(hourStr));
				mHourSpaceView.setText(Utils.toPersianNumbers(hourStr));
				mMinuteView.setText(Utils.toPersianNumbers(minuteStr));
				mMinuteSpaceView.setText(Utils.toPersianNumbers(minuteStr));
			}
			else if(mMode == ENGLISH)
			{
	...
	private int getAmOrPmKeyCode(int amOrPm) {
		...
		if (mAmKeyCode == -1 || mPmKeyCode == -1) {
			...
			for (int i = 0; i < Math.max(mAmText.length(), mPmText.length()); i++) {
				if(mMode == PERSIAN)
				{
					amChar = mAmText.charAt(i);
					pmChar = mPmText.charAt(i);
				}
				else
				{
	...
	public void setPersianAMText(String persianAMText)
	{
		this.mPersianAMText = persianAMText;
		mAmText = mPersianAMText;
	}

	public void setPersianPMText(String persianPMText)
	{
		this.mPersianPMText = persianPMText;
		mPmText = mPersianPMText;
	}

	public void setDoneButtonText(String mDoneButtonText)
	{
		this.mDoneButtonText = mDoneButtonText;
		mDoneButton.setText(this.mDoneButtonText);
	}
