...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
	...
	private String mPersianAMText;
	private String mPersianPMText;

	public interface OnValueSelectedListener {
		void onValueSelected(int pickerIndex, int newValue, boolean autoAdvance, boolean released);
	}
	...
	public RadialPickerLayout(Context context, AttributeSet attrs) {
		...
		mAmPmCirclesView.setMode(mMode);
	 ...
	 * @param is24HourMode
	 * @param mPersianPMText 
	 * @param mPersianAMText 
	 */
	public void initialize(Context context, int initialHoursOfDay, int initialMinutes,
			boolean is24HourMode, boolean vibrate, String mPersianAMText, String mPersianPMText, Typeface mTypeface) {
		if (mTimeInitialized) {
		...
		mAmPmCirclesView.setYpeface(mTypeface);
		
		this.mPersianAMText = mPersianAMText;
		this.mPersianPMText = mPersianPMText;
		...
		if (!mHideAmPm) {
			mAmPmCirclesView.initialize(context, initialHoursOfDay < 12 ? AM : PM, mPersianAMText, mPersianPMText, mMode);
		...
		for (int i = 0; i < 12; i++) {
			if(mMode == PersianTimePickerDialog.PERSIAN)
			{
				hoursTexts[i] = Utils.toPersianNumbers((is24HourMode ?
						String.format("%02d", hours_24[i]) : String.format("%d", hours[i])));
				innerHoursTexts[i] = Utils.toPersianNumbers(String.format("%d", hours[i]));
				minutesTexts[i] = Utils.toPersianNumbers(String.format("%02d", minutes[i]));
			}
			else if(mMode == PersianTimePickerDialog.ENGLISH)
			{
		...
		mHourRadialTextsView.initialize(res,
				hoursTexts, (is24HourMode ? innerHoursTexts : null), mHideAmPm, true, mTypeface);
		mHourRadialTextsView.invalidate();
		mMinuteRadialTextsView.initialize(res, minutesTexts, null, mHideAmPm, false, mTypeface);
	...
	public boolean performAccessibilityAction(int action, Bundle arguments) {
		...
		if (changeMultiplier != 0) {
			...
			mListener.onValueSelected(currentItemShowing, value, false, true);
			return true;
		}
	...
	public void setYpeface(Typeface mTypeface)
	{
		this.mTypeface = mTypeface;
	}
	...
	public void setPersianAMText(String mPersianAMText)
	{
		this.mPersianAMText = mPersianAMText;
	}

	public void setPersianPMText(String mPersianPMText)
	{
		this.mPersianPMText = mPersianPMText;
	}
}
